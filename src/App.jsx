  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Stage 2: Determine target year from route param or use latest
        const targetDate = date || (index.dates.length > 0 ? index.dates[index.dates.length - 1].date : null)
        const targetYear = targetDate ? targetDate.split('-')[0] : index.latestYear
        ...
        if (targetDate && yearData[targetDate]) {
          ...
          if (!date) {
            navigate(`/comic/${targetDate}`, { replace: true })
          }
        } else {
...
function ArticlesRoute() {
  ...
  return <Blog onArticleSelect={handleArticleSelect} />
}
...
function ArticleRoute() {
  ...
  return <Article articleId={articleId} onBack={handleBack} />
}
